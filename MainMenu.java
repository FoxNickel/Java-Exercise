...
public class MainMenu extends JFrame {
	...
	public MainMenu(DataBase dataBase) {
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					String select = "select *from IncomeAndSpending where rtype = '"
							+ selsectTypeOfIO.getSelectedItem().toString().trim() + "' AND uid = '"+dataBase.userName+"'";
				...
				if(!isDate(inputStartTime.getText())||!isDate(inputEndTime.getText())){
					...
					try {
						...
						String select = "select *from IncomeAndSpending where rdate >= "
								...
								+ inputEndTime.getText().toString().trim() + "' AND uid = '"+dataBase.userName+"'";
						ResultSet rs = statement.executeQuery(select);
						tableModel.setRowCount(0);
						while (rs.next()) {
							String id = rs.getString("id");
		...
		try {
			Connection conn = DriverManager.getConnection(dataBase.URL, dataBase.USERNAME, dataBase.PASSWORD);
			java.sql.Statement statement = conn.createStatement();
			String income = "select * from IncomeAndSpending where rtype = '收入' AND uid = '"+dataBase.userName+"'";
			ResultSet rs1 = statement.executeQuery(income);
			while (rs1.next()) {
				int balance = rs1.getInt("bal");
			...
			while(rs.next()){
				...
				String id = rs.getString("id");
				String date = rs.getString("rdate");
				String type1 = rs.getString("rtype");
				String item = rs.getString("ritem");
				tableModel.addRow(new Object[]{id,date,type1,item,balance});
			}
			String outcome = "select *from IncomeAndSpending where rtype = '支出' AND uid = '"+dataBase.userName+"'";
			ResultSet rs2 = statement.executeQuery(outcome);
			while (rs2.next()) {
				int balance = rs2.getInt("bal");
